...
do
	...
	then
		/usr/local/bin/anaconda2/bin/python class_project_scripts/03a2_bcftools_haps_to_LAMP-LD_haps.py --hap.gz ${i} --haps ${i::-7} --adm #admixed haplotypes are special for some reason
		sed -i -e "s/\\*//g" ${i::-7}
	else	
		/usr/local/bin/anaconda2/bin/python class_project_scripts/03a2_bcftools_haps_to_LAMP-LD_haps.py --hap.gz ${i} --haps ${i::-7}
		sed -i -e "s/\\*//g" ${i::-7}
	fi
done
...
then
	...
	/usr/bin/time ~/software/LAMPLD-v1.0/bin/unolanc 300 15 ${pos} ${out}CEU_reference.haps ${out}YRI_reference.haps ${out}NAT_reference.haps ${out}admixed.haps ${out}admixed_est.txt #version of LAMP-LD for 2-way admixture
elif [ ${direction_of_admixture} == 2 ]
then
	echo "running two-way estimation"
	if [ -e ${out}NAT_reference.haps ]
	then
		/usr/bin/time ~/software/LAMPLD-v1.0/bin/unolanc2way 300 15 ${pos} ${out}CEU_reference.haps ${out}NAT_reference.haps ${out}admixed.haps ${out}admixed_est.txt
	else
		/usr/bin/time ~/software/LAMPLD-v1.0/bin/unolanc2way 300 15 ${pos} ${out}CEU_reference.haps ${out}YRI_reference.haps ${out}admixed.haps ${out}admixed_est.txt
	fi
fi
 #use default window size of 300 and default number of states HMM 15
perl ~/software/LAMPLD-v1.0/convertLAMPLDout.pl ${out}admixed_est.txt ${out}admixed_est.long #convert from compact to long format
